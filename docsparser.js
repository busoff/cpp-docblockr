...
DocsParser.prototype.format_function = function(name, args, retval, options) {
    ...
    out.push(util.format("@brief ${%d: %s}", ++cursor, description));
    ...
    if(args) {
        ...
        for (i = 0; len = parsed_args.length, i < len; i++) {
            ...
            if(this.editor_settings.param_description) {
                format_str += util.format(' ${%d:[description]}', ++cursor);
            }
            if(this.settings.typeInfo) {
    ...
    if(ret_type !== null) {
        ...
        if(this.settings.typeInfo) {
            if (this.settings.curlyTypes) {
                type_info = util.format(' {${%d:%s}}', ++cursor, ret_type || '[type]');
            } else {
                type_info = util.format(' ${%d:%s}', ++cursor, ret_type || '[type]');
            }
        }
        ...
        if (this.editor_settings.return_description) {
            format_str = '%s%s %s${0:[description]}';
            var third_arg = '';
...
DocsParser.prototype.get_definition = function(line_reader) {
    var maxLines = 25;
